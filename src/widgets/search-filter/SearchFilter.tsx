...
const SearchFilter = () => {
  ...
  return (
    <div
      className={cn(
        'search__filter border border-solid border-[#ddd] rounded-lg p-[16px]',
      )}
    >
      ...
      <div className={cn('button__wrapper mt-[20px] flex gap-2')}>
        <button
          onClick={handleReset}
          className={cn(
            'px-[18px] py-[8px] border border-solid border-[#ccc] rounded-[6px] text-[14px] cursor-pointer',
          )}
        >
          초기화
        </button>
        <button
          className={cn(
            'px-[18px] py-[8px] bg-[#4154ff] text-white rounded-[6px] text-[14px] cursor-pointer',
          )}
        >
          1개 검색
        </button>
